...
class SQLAlchemyModelConverter:
    ...
    def _get_sqla_type(
        ...
        if optional and len(python_type.__args__) == 2:
            (python_type,) = [a for a in python_type.__args__ if a is not NoneType]
        ...
        if mapped_type is None:
            raise TypeError(
                f"Unknown type {python_type}, cannot find a suitable SQLAlchemy mapping."
            )
        return mapped_type, optional, list_

    @staticmethod
    def _get_related_entity(
        entity: Type[PythonEntityModel], name: str
    ) -> Type[PythonEntityModel]:
    ...
    def convert(self, entity: Type[PythonEntityModel]):
        ...
        for attr in entity.get_attributes():
            if (
                parent
                and attr in parent.get_attributes()
                and attr not in entity.get_primary_key()
            ):
                continue
            ...
            if isinstance(type_, str):
                ...
                attrs[attr] = Column(
                    attr, MutableList.as_mutable(JSON), nullable=optional
                )
            else:
                if attr in entity.get_primary_key():
                    ...
                    attrs[attr] = Column(
                        attr, type_, primary_key=True, autoincrement=autoincrement
                    )
                else:
        ...
        if parent_name:
            attrs["__mapper_args__"] = {
                ...
                "inherit_condition": and_(
                    *[
                        attrs[a] == getattr(parent.sqla_model, a)
                        for a in inherited_attrs
                    ]
                ),
            }
        ...
        return type(
            entity.__name__, (parent.sqla_model if parent else self.base,), attrs
        )
