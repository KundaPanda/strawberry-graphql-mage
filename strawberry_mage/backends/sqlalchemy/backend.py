...
class SQLAlchemyBackend(DataBackendBase):
    ...
    def _is_nullable(col: Union[ColumnProperty, RelationshipProperty]):
        if isinstance(col, ColumnProperty):
            return getattr(col, "nullable", False) or getattr(
                col.expression, "nullable", False
            )
    ...
    def get_attributes(
        ...
        all_ = [
            a.key
            for a in (
                inspection.mapper.attrs
                if isinstance(inspection, AliasedInsp)
                else inspection.attrs
            )
        ]
    ...
    def get_attribute_types(
        ...
        return {
            ...
            for attr in self.get_attributes(model)
        }
