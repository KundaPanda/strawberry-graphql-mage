    ...
    async def resolve(self, model: Type[PythonEntityModel], operation: GraphQLOperation, info: Info, data: Any,
        ...
        if isinstance(dataset, list):
            if operation == GraphQLOperation.DELETE_MANY:
                return model.get_strawberry_type().delete_one.type(affected_rows=len(dataset))
            if operation in {GraphQLOperation.UPDATE_MANY, GraphQLOperation.CREATE_MANY}:
                types = model.get_strawberry_type().graphql_input_types
                filter_type = dataclasses.make_dataclass('FilterStub', {'id': IntegerFilter})
                data = types['query_many_input'](page_size=UNSET,
                                                 filters=[filter_type(
        ...
        if isinstance(dataset, dict):
            if operation == GraphQLOperation.DELETE_ONE:
                return model.get_strawberry_type().delete_one.type(affected_rows=dataset.get('affected_rows', 1))
