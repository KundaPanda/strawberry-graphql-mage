...
class ModuleBoundStrawberryAnnotation(StrawberryAnnotation):
    ...
    def namespace(self, value):
        ...
        :param value: namespace as a dictionary
        :return: None
        """
        return
...
class IDataBackend(abc.ABC, Generic[TEntity]):
    ...
    @abc.abstractmethod
    def get_strawberry_field_type(self, type_: StrawberryAnnotation) -> Type:
        """
    ...
    def get_children_class_names(self, model: Type[TEntity]) -> Optional[List[str]]:
        ...
        :param model: model to inspect
        :return: list of (sub)class names
        """
        raise NotImplementedError
    ...
    async def resolve(self, model: Type[TEntity], operation: GraphQLOperation, info: Info, data: Any, *args,
                      **kwargs) -> Any:
        """
        Called for resolving a graphql operation
...
class ISchemaManager(abc.ABC):
    ...
    def backend(self) -> IDataBackend[ManagedEntity]:
        """
        The backend used for the entity models
...
class IEntityModel(abc.ABC):
    ...
    def get_strawberry_type(cls) -> StrawberryModelType:
        """
        The strawberry type for a model

        :return: strawbery type
        """
        raise NotImplementedError
    ...
    def get_operations(cls) -> Set[GraphQLOperation]:
        """
        All operations which are (should be) defined for the model
    ...
    @classmethod
    @abc.abstractmethod
    def get_children_class_names(cls) -> List[str]:
        """
        Get names of all child classes of an entity including the entity itself
    ...
    def pre_setup(cls, manager: ISchemaManager) -> None:
        """
        Callback when entity models are being registered in a SchemaManager
    ...
    def post_setup(cls) -> None:
        """
        Callback after all models have been initialized and schema creation is in progress
