...
def make_fk(remote: Union[str, Type[DeclarativeMeta]], remote_pk='id', optional=False, back_populates=None,
    ...
    fk = Column(Integer, ForeignKey(f'{remote_table}.{remote_pk}', use_alter=True), nullable=optional, **foreign_kwargs)
    rel = relationship(remote, back_populates=back_populates, foreign_keys=[fk], **rel_kwargs)
    return fk, rel


def make_composite_fk(remote: Union[str, Type[DeclarativeMeta]], remote_keys: Tuple[str, ...], model_name: str,
                      rel_name: str, optional=False, back_populates=None, foreign_kwargs=None, rel_kwargs=None) \
    ...
    for key in remote_keys:
        foreign_keys.append((key, Column(f'{rel_name}_{key}', Integer, nullable=optional, **foreign_kwargs)))
    rel = relationship(remote, back_populates=back_populates, foreign_keys=[f[1] for f in foreign_keys],
