import json
from typing import Type, Optional, Iterable, Dict, Any, List, Callable
...
class JSONBackend(PythonBackend):
    ...
    def _create_entity(self, mappings: Dict[str, _SQLAlchemyModel], original: dict):
        data = self._extract_attributes(mappings, original)
    ...
    def _extract_attributes(self, mappings: Dict[str, _SQLAlchemyModel], entry: dict):
        results: Dict[str, Any] = {}
        for a, attr in entry.items():
            if attr and isinstance(attr, dict):
                key = self._get_entry_key(attr)
                if key not in mappings:
                    mappings[key] = self._create_entity(mappings, attr)
                ...
                for e in attr:
                    if isinstance(e, dict):
                        ...
                        if key not in mappings:
                            mappings[key] = self._create_entity(mappings, e)
    ...
    def _get_entry_key(self, entry: dict):
        ...
        return json.dumps({'__model__': model.__name__, **{k: entry.get(k, None) for k in model.get_primary_key()}})

    def _build_dataset(self, dataset: Iterable[dict]):
        mappings: Dict[str, _SQLAlchemyModel] = {}
        for entry in dataset:
            key = self._get_entry_key(entry)
            entity = self._create_entity(mappings, entry)
            mappings[key] = entity
        return mappings
